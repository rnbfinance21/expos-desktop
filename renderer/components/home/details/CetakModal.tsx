  ...
  const _sendToKitchenAdditional = () => {
    if (ipcRenderer) {
      ...
      if(data.details.filter(e => e.type === 1).length > 0) {
        ...
        Toast.fire("Peringatan!", "Tidak terdapat item tambahan pada pesanan ini", "warning");
      }
    }
  };
  ...
  return (
    <MyModal title="Pilih Opsi Cetak" show={show} onClose={onClose}>
      <div className="mt-4">
        ...
        <div onClick={_sendToKitchenAdditional} className="flex flex-row p-4 bg-gray-100 cursor-pointer mb-2 items-center">
