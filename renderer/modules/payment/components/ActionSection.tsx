...
import { savePayment, updatePayment, voidPayment } from "@/modules/order/api";
import {
    OrderDetail,
    SavePaymentParams,
    UpdatePaymentParams,
...
import { DetailVariant } from "@/modules/order/type";
...
const ActionSection = () => {
    ...
    const saveMutation = useMutation(
        (params: SavePaymentParams) => savePayment(token, params),
        {
            retry: false,
            onSuccess: async (res) => {
                if (ipcRenderer) {
    ...
    const voidMutation = useMutation(
        (params: VoidPaymentParams) => voidPayment(token, params),
        {
            onSuccess: (res) => {
                if (ipcRenderer) {
                    ipcRenderer.send(
        ...
        if (validationSimulate() && id !== null) {
            if (ipcRenderer) {
                let simluateData: OrderDetail = {
                    ...
                    kategori_payment_name:
                        paymentTypeList[paymentType - 1].name,
                    kode_transaksi: "-",
                    name: identity.name,
                    no_bill: identity.no_bill,
                    outlet_id: null,
                ...
                mutationLogs.mutate(
                    "[Belum Lunas][Simulasi Print] Cetak struk"
                );
            }
        } else {
            Toast.fire({
