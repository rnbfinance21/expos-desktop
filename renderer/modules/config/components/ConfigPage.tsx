...
import { regenerateCode } from "@/modules/auth/api";

const LIST_SIZE = [
  { label: "Custom 80mm", value: "1" },
  { label: "80mm", value: "2" },
];
...
const ConfigPage = () => {
  ...
  const [printers, setPrinters] = useState<{ value: string; label: string }[]>(
    []
  );
  ...
  const _onPressTest = (type = 1) => {
    if (ipcRenderer) {
      if (type === 1 && printCashier !== "" && printCashierSize !== "") {
        ipcRenderer.send(
          "print-testing",
          printCashier,
          parseInt(printCashierSize, 10)
        );
      } else if (type === 2 && printKitchen !== "" && printKitchenSize !== "") {
        ipcRenderer.send(
          "print-testing",
          printKitchen,
  ...
  const mutation = useMutation(() => regenerateCode(token), {
    onSuccess: () => {
      refetch();
  ...
  useEffect(() => {
    if (ipcRenderer) {
      const getPrinters: PrinterInfo[] = ipcRenderer.sendSync("printer-list");
      setPrinters(
          ...
          <div className="px-4">
            ...
            <div className="flex flex-row gap-4 mt-4">
              <Select
                ...
                options={printers}
                onChange={(e) => e && setPrintCashier(e.value)}
                value={printers.find((e) => e.value === printCashier) ?? null}
                className="w-[300px]"
              />
            </div>
          </div>
          <div className="px-4">
            ...
            <div className="flex flex-row gap-4 mt-4">
              <Select
                ...
                options={LIST_SIZE}
                onChange={(e) => e && setPrintCashierSize(e.value)}
                value={
                  LIST_SIZE.find((e) => e.value === printCashierSize) ?? null
                }
                className="w-[300px]"
              />
            </div>
          ...
          <div className="px-4">
            ...
            <div className="flex flex-row gap-4 mt-4">
              <Select
                ...
                placeholder="Pilih Printer"
                options={printers}
                onChange={(e) => e && setPrintKitchen(e.value)}
                value={printers.find((e) => e.value === printKitchen) ?? null}
                className="w-[300px]"
              />
            </div>
          </div>
          <div className="px-4">
            ...
            <div className="flex flex-row gap-4 mt-4">
              <Select
                ...
                placeholder="Pilih Ukuran"
                options={LIST_SIZE}
                onChange={(e) => e && setPrintKitchenSize(e.value)}
                value={
                  LIST_SIZE.find((e) => e.value === printKitchenSize) ?? null
                }
                className="w-[300px]"
              />
            </div>
